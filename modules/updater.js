            ...
            request({
                ...
                if (error || response.statusCode !== 200) {
                    ...
                    try {
                        ...
                        else vTag = updateData.tag_name || updateData.version;
                        if (semver.parse(vTag)) {
                            remoteVersions.set(this, vTag);
                            remoteChangelog.set(this, updateData.body);
                            resolve({
                                version: vTag,
                                changelog: updateData.body || "N/A"
                            });
                        } else {
                            reject({
                                error: new InvalidSemverFormatError(vTag),
                                type: "SEMVERFORMAT"
                            });
                        }
                    } catch (err) {
    ...
    checkUpdate() {
        return new Promise((resolve, reject) => {
            this.fetchUpdateData().then(() => {
                resolve({
                    newerVersion: this.isUpdateAvailable() || this.isUpgradeAvailable(),
                    type: this.isUpgradeAvailable() ? "UPGRADE" : "UPDATE"
                });
            }).catch(reject);
        });
    }
